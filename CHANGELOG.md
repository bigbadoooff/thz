...
- **The fan stayed unavailable after one failed read.** A read error while
  ...
  blocks its stage is computed from, and no longer shows a stage from a
...
- **A clock correction counted as done even when it did not stick.**
  ...
  moved on by a minute during the correction, or rolled over to the next
  hour while it was read back, still counts as set. A restore reports a
  clock the read-back does not confirm as failed.
...
- **Parameter backups lost the party end.** The party register holds the
  ...
  both; older backups still restore the start.
